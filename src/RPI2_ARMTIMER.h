...
#ifndef RPI2_ARMTIMER_H
#define RPI2_ARMTIMER_H
...
#define ARMTIMER_BASE        (PERIPHERAL_BASE + 0xB000)
...
#define ARMTIMER_IRQ_CLR     (ARMTIMER_BASE + 0X40C)
#define ARMTIMER_IRQ_RAW     (ARMTIMER_BASE + 0X410)
#define ARMTIMER_IRQ_MASK    (ARMTIMER_BASE + 0X414)
#define ARMTIMER_RELOAD      (ARMTIMER_BASE + 0X418)
#define ARMTIMER_PREDIV      (ARMTIMER_BASE + 0X41C)
#define ARMTIMER_RNNG_CTR    (ARMTIMER_BASE + 0X420)
...
typedef struct {
  ...
  /** The free running counter is not present in the SP804. The free
      running counter is a 32 bits wide read only register. The
      register is enabled by setting bit 9 of the Timer control
      register. The free running counter is incremented immediately
      after it is enabled. The timer can not be reset but when
      enabled, will always increment and roll-over.
      
      The free running counter is also running from the APB clock and
      has its own clock pre-divider controlled by bits 16-23 of the
...
extern void armTimerStart (void);
extern void armTimerStop (void);
extern void armTimerLoad (uint32_t ld);
extern void armTimerReload (uint32_t ld);
extern char armTimerInterrupted (void);
extern void armTimerInterruptClear();


#endif /* RPI2_ARMTIMER_H */
